...
class QuestionModelTest(TestCase):
    ...
    def test_was_published_rencently_with_future_question(self):
        ...
        self.assertIs(future_question.was_published_rencently(), False)

    def test_was_published_recently_with_old_question(self):
        time = timezone.now() - datetime.timedelta(days=1, seconds=1)
        old_question = Question(pub_data=time)
        self.assertIs(old_question.was_published_rencently(), False)

    def test_was_published_recently_with_rencent_question(self):
        time = timezone.now() - datetime.timedelta(hours=23, minutes=59, seconds=59)
        recent_question = Question(pub_data=time)
        self.assertIs(recent_question.was_published_rencently(), True)
